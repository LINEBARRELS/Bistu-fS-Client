...
class Tag extends React.Component {
  ...
  render() {
    ...
    content.forEach((item, index) => {
      result.push(
        <div className='tag' key={index} data-index={index} onClick={this.removeTag.bind(this)}>{item}</div>
      )
    })
    ...
    return <div className='tag-container'>
      ...
      <div className='content'>
        <CSSTransitionGroup transitionName="tagAni" transitionEnterTimeout={150} transitionLeaveTimeout={150}>
          {result}
        </CSSTransitionGroup>
      </div>
    </div>
