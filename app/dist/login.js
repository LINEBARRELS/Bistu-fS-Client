...
function checkLetter(str){
    ...
    var result = str.match(reg)===null?true:false;
    return result;
}
...
user.addEventListener('change',function(event){
    ...
    try{
       if(!checkLetter(this.value)) throw '包含非法字符'
       if(!checkLength(this.value)) throw '长度不科学'
    }catch(err){
...
cofm.addEventListener('change',function(event){
    ...
    try{
       if(!checkLetter(this.value)) throw '包含非法字符'
       if(!checkLength(this.value)) throw '长度不科学'
       if(this.value!==pass.value) throw '两次输入密码不相同,请重新输入'
    }catch(err){
        this.classList.add('err');
        this.nextElementSibling.innerText=err;
    }
